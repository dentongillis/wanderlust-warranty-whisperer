      ...
      <table className="w-full text-xs">
        <thead>
          <tr className="bg-gray-50 dark:bg-gray-800 border-b border-gray-100 dark:border-gray-700">
            ...
            <th className="px-3 py-2 text-left font-medium text-gray-600 dark:text-gray-300">Claim ID</th>
            <th className="px-3 py-2 text-left font-medium text-gray-600 dark:text-gray-300">Model</th>
            <th className="px-3 py-2 text-left font-medium text-gray-600 dark:text-gray-300">Dealer</th>
            <th className="px-3 py-2 text-left font-medium text-gray-600 dark:text-gray-300">Status</th>
            <th className="px-3 py-2 text-left font-medium text-gray-600 dark:text-gray-300">Total</th>
        ...
        <tbody>
          {recentClaimsData.map((claim, index) => (
            <tr key={index} className="border-b border-gray-100 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800">
              <td className="px-3 py-3 text-gray-600 dark:text-gray-300">{claim.date}</td>
              <td className="px-3 py-3 font-medium text-gray-800 dark:text-gray-200">{claim.claimId}</td>
              <td className="px-3 py-3 text-gray-600 dark:text-gray-300">{claim.model}</td>
              <td className="px-3 py-3 text-gray-600 dark:text-gray-300">{claim.dealer}</td>
              <td className="px-3 py-3">
                ...
                {claim.status === 'pending' && (
                  <Badge variant="outline" className="bg-yellow-100 text-yellow-800 dark:bg-yellow-900/30 dark:text-yellow-300 border-yellow-200 dark:border-yellow-800/30 text-xs px-1.5 py-0">
                    Pending
                  </Badge>
                )}
                {claim.status === 'denied' && (
              ...
              <td className="px-3 py-3 font-medium text-gray-800 dark:text-gray-200">{claim.total}</td>
